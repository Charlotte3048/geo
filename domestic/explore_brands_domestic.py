...
def get_brands_from_text_with_ai(client: openai.OpenAI, text: str, model: str) -> list:
    ...
    system_prompt = """
            你是一个专业的市场分析师。你的任务是从给定的中文文本中，提取所有明确的零食或饮料品牌名称。
            规则:
            1. 只返回品牌名称，例如: "三只松鼠", "良品铺子", "元气森林", "康师傅"。
            2. 忽略非品牌概念，例如: 品类 (如 "薯片", "坚果", "苏打水"), 口味 (如 "麻辣味", "海苔味"), 描述词 (如 "好吃", "健康"), 渠道 (如 "超市", "便利店")。
            3. 若提及品牌与其下属产品同时出现，仅返回品牌名称。例如: "我喜欢喝康师傅的冰红茶" → 返回 ["康师傅"]。
            4. 返回 JSON 数组，例如: ["奥利奥", "卫龙", "农夫山泉"]；如果未识别到任何品牌名称，则返回 []。
            """
